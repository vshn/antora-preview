...
CI runs it on `amd64` and on `arm64`, each on a native runner, because the
image is published for both and most people run it on an arm64 Mac. Nothing is
published or released unless both pass.
...
Pushing to `master` builds, smoke tests, publishes and then tags the image.
The version is the version of the `ghcr.io/vshn/antora:x.y.z` base image in the
`Dockerfile`, plus the revision in the `REVISION` file:

----
FROM ghcr.io/vshn/antora:3.1.14   +   REVISION 1   ->   3.1.14.1
----

Renovate bumps the base image on its own, which produces a new version without
any further action. When a change to this repository alone needs to be
released, bump `REVISION`. It does not have to be reset when the base image
moves; `3.1.14.1` sorts before `3.1.15.1` either way.

If the version already has a tag, the release step is skipped and the existing
